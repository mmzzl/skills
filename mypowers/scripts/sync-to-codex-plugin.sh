...
#
# Sync this cospowers checkout → the Codex plugin marketplace fork.
...
# plugins/cospowers/ when absent, then copies the tracked plugin files from
...
FORK="<github-org>/<codex-plugins-repo>"   # TODO: set target Codex plugin marketplace fork
DEFAULT_BASE="main"
DEST_REL="plugins/cospowers"
...
UPSTREAM_REMOTE_URL="$(cd "$UPSTREAM" && git remote get-url origin 2>/dev/null || echo 'unknown')"
...
if [[ $BOOTSTRAP -eq 1 ]]; then
  SYNC_BRANCH="bootstrap/cospowers-${UPSTREAM_SHORT}-${TIMESTAMP}"
else
  SYNC_BRANCH="sync/cospowers-${UPSTREAM_SHORT}-${TIMESTAMP}"
fi
...
if [[ $BOOTSTRAP -eq 1 ]]; then
  echo "Mode:     BOOTSTRAP (creating plugins/cospowers/ when absent)"
fi
echo ""
...
if [[ $BOOTSTRAP -eq 1 ]]; then
  COMMIT_TITLE="bootstrap cospowers v$UPSTREAM_VERSION from upstream main @ $UPSTREAM_SHORT"
  PR_BODY="Initial bootstrap of the cospowers plugin from upstream \`main\` @ \`$UPSTREAM_SHORT\` (v$UPSTREAM_VERSION).

Creates \`plugins/cospowers/\` by copying the tracked plugin files from upstream, including \`.codex-plugin/plugin.json\` and \`assets/\`.

Run via: \`scripts/sync-to-codex-plugin.sh --bootstrap\`
Upstream commit: ${UPSTREAM_REMOTE_URL}/commit/$UPSTREAM_SHA
...
This is a one-time bootstrap. Subsequent syncs will be normal (non-bootstrap) runs using the same tracked upstream plugin files."
  ...
  COMMIT_TITLE="sync cospowers v$UPSTREAM_VERSION from upstream main @ $UPSTREAM_SHORT"
  PR_BODY="Automated sync from cospowers upstream \`main\` @ \`$UPSTREAM_SHORT\` (v$UPSTREAM_VERSION).
...
Upstream: ${UPSTREAM_REMOTE_URL}/commit/$UPSTREAM_SHA
Branch:   $SYNC_BRANCH"
